import { BiomeId } from "./biome-id";
import { SpeciesId } from "./species-id";
...
let pokemon_list: { [key: string]: string[][] } = {};

for (const biomeId in biomePokemonPools) {
    console.log(`Biome: ${BiomeId[biomeId]}`);
    for (const rarity in biomePokemonPools[biomeId]) {
        console.log(`  Rarity: ${BiomePoolTier[rarity]}`);
        for (const time in biomePokemonPools[biomeId][rarity]) {
            console.log(`    Time: ${TimeOfDay[time]}`);
            for (const dict in biomePokemonPools[biomeId][rarity][time]) {
                const species = biomePokemonPools[biomeId][rarity][time][dict];

                if (typeof species === 'object') {
                    for (const level in species) {
                        const specie = species[level].map(s => SpeciesId[s] || s).join(', ');
                        console.log(`      Species: ${specie}`);
                        if (!pokemon_list[specie]) {
                            pokemon_list[specie] = [[BiomeId[biomeId], BiomePoolTier[rarity], TimeOfDay[time]]];
                        } else {
                            pokemon_list[specie].push([BiomeId[biomeId], BiomePoolTier[rarity], TimeOfDay[time]]);
                        }
                    }
                } else {
                    const specie = SpeciesId[species] || species;
                    console.log(`      Species: ${specie}`);
                    if (!pokemon_list[specie]) {
                        pokemon_list[specie] = [[BiomeId[biomeId], BiomePoolTier[rarity], TimeOfDay[time]]];
                    } else {
...
fs.writeFileSync('pokemon_spawn.json', JSON.stringify(pokemon_list), 'utf8');
