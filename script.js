...
function handleSearch(searchInput, container) {
    ...
    for (const item of items) {
        ...
        if (index > -1) {
            ...
            const highlightedHTML = originalText.substring(0, index) +
                                  '<b>' + originalText.substring(index, matchEndIndex) + '</b>' +
...
for (const pokemonName in allPokemonData) {
    ...
    const targetBiomesContainer = document.getElementById('targetBiomesContainer');
    const avoidBiomeSearchInput = document.getElementById('avoidBiomeSearch');
    const avoidBiomesContainer = document.getElementById('avoidBiomesContainer');

    pokemonSearchInput.addEventListener('input', () => handleSearch(pokemonSearchInput, pokemonListContainer));
